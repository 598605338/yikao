...
var dividePage = {
    ...
    getLink: function (n) {
        ...
        //第2页就是http://www.baidu.com/news/20131212_2.html
    ...
    getLink2: function (pageIndex,pageSize) {
    	//通过提交form一并提交请求参数
    	return submitPageForm(pageIndex,pageSize);
    },
    ...
    init: function (config) {
        ...
        this.pageSize = isNaN(config.pageSize) ? 10 : parseInt(config.pageSize);
        ...
        if (config.getLink2 && typeof (config.getLink2) == 'function') { this.getLink2 = config.getLink2; }
        //验证
    ...
    generPageHtml: function () {
        ...
        if (this.isGoPage) {
            gopage_info = '&nbsp;转到<span id="go_page_wrap" style="display:inline-block;width:44px;height:18px;border:1px solid #DFDFDF;margin:0px 1px;padding:0px;position:relative;left:0px;top:5px;">' +
					'<input type="button" id="btn_go" onclick="dividePage.gopage();" style="width:44px;height:20px;line-height:20px;padding:0px;font-family:arial,宋体,sans-serif;text-align:center;border:0px;background-color:#0063DC;color:#FFF;position:absolute;left:0px;top:-1px;display:none;" value="确定" />' +
					'<input type="text" id="btn_go_input" onfocus="dividePage.focus_gopage()" onkeypress="if(event.keyCode<48 || event.keyCode>57)return false;" onblur="dividePage.blur_gopage()" style="width:42px;height:16px;text-align:center;border:0px;position:absolute;left:0px;top:0px;outline:none;" value="' + this.next + '" /></span>页';
        }
    ...
    generPageHtml2: function () {
        ...
        if (this.hasNext) {
            str_next = '<a href="' + this.getLink2(this.next,this.pageSize) + '" title="下一页">下一页</a>';
        } else {
            str_next = '<span class="disabled">下一页</span>';
        }
        ...
        if (this.total <= 8) {
            for (var i = 1; i <= this.total; i++) {
                if (this.pno == i) {
                    ...
                    str += '<a href="' + this.getLink2(i,this.pageSize) + '" title="第' + i + '页">' + i + '</a>';
                }
            }
        } else {
            if (this.pno <= 5) {
                ...
                str += '<a href="' + this.getLink2(1,this.pageSize) + '" title="第1页">1</a>';
                str += '<a href="' + this.getLink2(2,this.pageSize) + '" title="第2页">2</a>';
                str += dot;
    ...
    //生成分页控件Html
    getTotals: function (totalRecords,pg) {
    	var totalPage = 1;
		var pageSize=10;
		if(pg){
			pageSize=pg;
    	...
    	if(totalRecords){
			if(totalRecords%pageSize>0){
				totalPage=parseInt(totalRecords/pageSize)+1;
			}else{
