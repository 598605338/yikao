			...
			function delAll(type){
					var activity_ids="";
						...
						if(activity_ids){
							...
							window.location.href="<%=path%>/hotsell/delete?recordIds="+data1;
						}
			}
			
			function addNew(){
				window.location.href="<%=path%>/hotsell/toAdd";
			}
...
<body>
	...
	<div id="wrap">
				<header class="cont_section_header_bread">
					...
					<span style="color:#3bb3e0;">热销推荐</span>
				</header>
				<section class="cont_section_section">					
					<div class="goodsManage">
						<div class="cont_section_head">
						<form name="formain" id="formain" method="post" action="<%=path%>/hotsell/select">
							<div class="row">
								<input name="name" id="name" placeholder="活动名称" type="text" class="left" value="${query.name}"/>   
								<input class="datatime-input" placeholder="活动时间起" type="text" name="startTimeStr" id="startTimeStr" value="${query.startTimeStr}" />
								<input class="datatime-input" placeholder="活动时间止" type="text" name="endTimeStr" id="endTimeStr" value="${query.endTimeStr}" />
							...
							<div class="col-click">
								<span ><button type="button" onclick="document.getElementById('formain').submit();" class="left back_dblue col_white btn"><i class="icon-btn icon-search"></i>查询</button></span>
						...
						<div class="height compa" style="margin-bottom: 15px;">
							<span ><button class="left back_dblue btn" onclick="addNew()"><i class="icon-btn icon-add"></i>新增</button><button class="left back_dblue btn" onclick="delAll()"><i class="icon-btn icon-delete"></i>删除</button></span>
							<span class="right">共${query.totalNums}条活动信息&nbsp;</span>
						</div>	
						<thead style="background:#EBEBEB;">
							<th><input style="margin-left:30%" type="checkbox" name="all" id="all" onclick="doChoose()"/></th>
							<th>活动名称</th>
							<th>发布时间</th>
							<th>活动时间</th>
							<th>使用状态</th>
						...
						<tbody>
						<c:forEach var="item" items="${activityHotsellList}" varStatus="status">
						 	<tr class="goods_tr">
								<td><input style="margin-left:30%" type="checkbox" name="id" id="id" value="${item.id}" onclick="chooseSingle()"></td>
								<td>${item.name}</td>
								...
								<td><a href="<%=path%>/hotsell/toEdit?id=${item.id}"><i class="icon-op icon-op-edit"></i>修改</a>
								&nbsp;&nbsp;
								<c:choose>  
  										<c:when test="${item.online==0}">
  										<a class="mr10" href="<%=path%>/hotsell/updateStatus?id=${item.id}&online=1"><i class="icon-op icon-op-able"></i>启用</a>
   										</c:when>  
									    <c:when test="${item.online==1}">
  										<a class="mr10" href="<%=path%>/hotsell/updateStatus?id=${item.id}&online=0"><i class="icon-op icon-op-disable"></i>禁用</a>
   										</c:when> 
									</c:choose>
								</td>
							</tr>
	...
	var trim_Version=version[1].replace(/[ ]/g,""); 
	 if(browser=="Microsoft Internet Explorer" && trim_Version=="MSIE9.0") {
	  $(".compa").css("margin","15px 0px 0px 0px")
	}
	</script>
</html>
