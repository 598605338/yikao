...
<body>
	<div id="wrap">
				...
				<header class="cont_section_header_bread">
					...
					<span style="color:#3bb3e0;">品牌管理</span>
				</header>
				<section class="cont_section_section">					
					...
					<form name="form" id="formSubmit" action="select" method="post">
						<div class="row">
							<input type="text" name="name" id="name" placeholder="品牌名称" value="<c:out value="${query.name }"/>" class="td_text_w" />
						</div>
						<div class="col-click">
						...
						<div class="section_table">
							...
							<div class="click">
							<a href="javascript:void(0)" onclick="javascript:location.href='toAdd'"><span class="left back_dblue btn"><i class="icon-btn icon-add"></i>新增品牌</span></a>
						...
						<c:forEach var="item" items="${brandList }" varStatus="status">
							<tr class="goods_tr">
								...
								<td  style="width:10%;"><c:if test="${item.useStatus==false }">启用</c:if><c:if test="${item.useStatus==true }">禁用</c:if></td>
								<td  style="width:20%;">
...
function deleteRecord(){
	...
	$("[name='id']").each(function(i,item){
		if(item.checked==true)
			brandIds.push(item.value);
	});
	if(brandIds.length > 0){
		document.location.href="deleteBrand?brandIds=" + brandIds;
	}else{
		alert("请选择要删除的品牌");
	}
}
