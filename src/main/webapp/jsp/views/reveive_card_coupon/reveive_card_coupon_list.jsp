				...
				<section class="cont_section_section">					
					...
					<form name="form" id="fromSubmit" action="select" method="post">
						...
						<thead style="background:#ebebeb;">
							<th><input style="margin-left:22%;" type="checkbox" name="selAll" onclick="selAll(this);"/></th>
							<th>卡劵名称</th>
							<th>活动类型</th>
							<th>开始日期</th>
							<th>结束日期</th>
						...
						<tbody>
						<c:forEach var="item" items="${receiveCardCouponList }" varStatus="status">
							<tr class="goods_tr">
								...
								<td>
									...
									<a href="javascript:void(0)" class="mr10" onclick="javascript:location.href='updateUseStatus?id=<c:out value="${item.id }" />&useStatus=<c:out value="${item.useStatus }" />'"><c:if test="${item.useStatus==false }"><i class="icon-op icon-op-disable"></i>禁用</c:if><c:if test="${item.useStatus==true}"><i class="icon-op icon-op-able"></i>启用</c:if></a>
									<a href="javascript:void(0)" class="mr10" onclick="javascript:location.href='deleteReceiveCardCoupon?recordIds=<c:out value="${item.id }" />'" ><i class="icon-op icon-op-delete"></i>删除</a>
								</td>
							</tr>
						</c:forEach>
						</tbody>
...
function deleteRecord(){
	...
	if(recordIds.length > 0){
		document.location.href="deleteReceiveCardCoupon?recordIds=" + recordIds;
	}else{
		alert("请选择要删除的记录");
	}
}
