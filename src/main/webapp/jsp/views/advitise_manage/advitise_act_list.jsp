...
<c:out value="${message }" />
				...
				<section class="cont_section_section">					
					<div class="goodsManage">
						...
						<tbody>
						    <c:forEach var="actAdv" items="${actAdvlist}" varStatus="status">
						    <tr class="goods_tr">
								<td>${actAdv.activity_id}</td>
								<td>${actAdv.activity_name}</td>
								<td><img style="width:45px;" src="${actAdv.picture}"/></td>
								<td>${actAdv.activity_rule}</td>
								<td>
									<a href="<%=path%>/advManage/selectAyAdvInfoById?id=${actAdv.id}"><i class="icon-op icon-op-edit"></i>修改</a>&nbsp;&nbsp;
									<a href="<%=path%>/advManage/deleteAyAdvInfoById?id=${actAdv.id}"><i class="icon-op icon-op-delete"></i>删除</a>
								</td>
							</tr>
						      </c:forEach>
						     </tbody>
