...
<body>
	<div id="wrap">
				...
				<header class="cont_section_header_bread">
					...
					<span style="color:#3bb3e0;">标签管理</span>
				</header>
				<section class="cont_section_section">					
					...
					<form name="form" id="formSubmit" action="select" method="post">
						<div class="row">
							<input type="text" name="name" placeholder="标签名称" id="name" value="<c:out value="${query.name }"/>" class="td_text_w" />
						...
						<tbody>
						<c:forEach var="item" items="${tagsList }" varStatus="status">
							<tr class="goods_tr">
								...
								<td  style="width:20%;" ><c:if test="${item.type==1 }">促销标签</c:if><c:if test="${item.type==2 }">商品标签</c:if></td>
								<td  style="width:40%;"><c:out value="${item.description }"/></td>
								<td  style="width:20%;">
									<a href="javascript:void(0)" onclick="javascript:location.href='toEdit?id=<c:out value="${item.id }" />'"><i class="icon-op icon-op-edit"></i>修改</a>
								</td>
							</tr>
						</c:forEach>
						</tbody>
...
function deleteRecord(){
	...
	$("[name='id']").each(function(i,item){
		if(item.checked==true)
			tagsIds.push(item.value);
	});
	if(tagsIds.length > 0){
		document.location.href="deleteTags?tagsIds=" + tagsIds;
	}else{
		alert("请选择要删除的标签");
	}
}
