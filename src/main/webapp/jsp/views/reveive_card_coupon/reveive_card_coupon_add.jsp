...
function doSubmit(){
	...
	if(publishNum==null || publishNum==''){
        document.saveForm.publishNum.focus();
...
<div id="wrap">
	...
	<div class="editPage">
		<div class="editLeft">
			...
			<div class="row-list">
				<label>发放时间：</label>
				<input style="width: 31%;" placeholder="发放时间起" type="text" name="publishStartTimeStr" id="publishStartTimeStr" value=""  class="datatime-input" />
				<input style="width: 32%;" placeholder="发放时间止" type="text" name="publishEndTimeStr" id="publishEndTimeStr" value=""  class="datatime-input" />
			</div>
			<div class="row-list">
				<label>发放方式：</label>
				<input style="border:none;" class="left" type="radio" name="publishType" id="publishType" value="1" checked="checked" /><span style="margin-top: 2px;">投放到领券中心</span>
			</div>
			<div class="row-list">
				<label>卡券：</label>
				<input type="text" style="width: 25%;" name="publishNum" id="publishNum" value=""  maxlength="4" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/><b style="float: left;">张</b>
				<input type="text" style="width: 23%;margin-right: 1.5%;" name="cardName" id="cardName" readonly="readonly" value="" /><a href="<%=basePath%>/cardCoupon/select?popupFlg=1&params=cardCouponId,cardName" target="popupWin" onclick="popupProOpen();"><span style="margin: 0;position: relative;margin-top: 1px;" class="back_dblue btn"><i class="icon-btn icon-enable"></i>选择</span></a>
			</div>
		</div>
		<div class="line"></div>
		<div class="editRight">
	...
	<div class="bgbtn">
		<!-- <input style="margin-left:-222%;" type="button" value="保存" class="left btn back_dblue" onclick="doSubmit()" /> 
		<input style="margin-left:-166%;" type="button" value="返回" class="left btn back_dblue" onclick="doBack()" /> -->
		<button type="button" onclick="doSubmit()" class="left back_dblue col_white btn"><i class="icon-btn icon-save"></i>保存</button>
		<button type="button" class="left back_dblue col_white btn" onclick="doBack();"><i class="icon-btn icon-goback"></i>返回</button>
	</div>
