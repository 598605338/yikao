	 ...
	 function cleanForm(){
		...
		//全选功能
			...
			function getproList2(){
				...
				var c=0;
				var d=0;
				var traproducts = new Array();
				$("#pro_item2 tr").each(function(i,item){
						var obj={};
						var productId = $(item).closest('tr').find("[name='product_id']").val();
						var pSendType = $(item).closest('tr').find("[name='p_send_type']").val();
						var pCode = $(item).closest('tr').find("[name='p_code']").val();
						var pName = $(item).closest('tr').find("[name='p_name']").val();
						var marketPrice = $(item).closest('tr').find("[name='p_price']").val();
						var pTradePrice =  $(item).closest('tr').find("[name='p_trade_price']").val();
						var quantity =  $(item).closest('tr').find("[name='quantity']").val();
						if(pTradePrice==null || pTradePrice==''){
						       a=1;
							}else{
								var reg = new RegExp("^[0-9]*$");
								var reg1 = /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/;
								if(!(reg.test(pTradePrice)||reg1.test(pTradePrice))){  
							       b=1;
							    } 
							}
						...
						obj.p_name=pName;
						obj.p_send_type=pSendType;
						obj.p_trade_price=pTradePrice;
						obj.quantity=quantity;
						obj.p_price=marketPrice;
						traproducts.push(obj);
				});
				...
				if(traproducts!=null&&traproducts.length){
					var tradstr=JSON.stringify(traproducts);
					$("#actTradeProducts").val(tradstr);
				}else{
					$("#actTradeProducts").val('');
				}
				return true;
			}
...
<c:out value="${message }" />
	<div id="wrap">
				<!-- <header class="cont_section_header">加钱换购</header> -->
				<header class="cont_section_header_bread">
					<i class="icon-op icon-op-address" style="margin-right:5px;"></i>当前位置：
					<a href="javascript:void(0);" class="topNavClick" style="color: #323232;">活动</a><span>&gt;</span>
					<a href="javascript:void(0);" class="leftNavClickList" style="color: #323232;">加钱换购</a><span>&gt;</span>
				...
				<section class="cont_section_section_border">					
					<div class="goodsManage">
						...
						<div class="editPage">
							<div class="editLeft">
								...
								<div class="row-list">
									<label>促销日期：</label>
									<input style="width:30%" class="datatime-input" type="text"  name="beginDate" id="beginDate" value="${activityInfo.beginDate}" >
									<p style=" float:left;width:3%;display:inline-block;text-align: center; margin-left: -1px; margin-right: 1px;line-height: 32px;">~</p>
									<input style="width:30%;" class="datatime-input" type="text"  name="endDate" id="endDate" value="${activityInfo.endDate}" >
									<input type="hidden" name="activity_id" readonly="readonly" value="${activityInfo.activity_id}"/>
									<input type="hidden" id="activity_type"  value=4 name="activity_type" />
								...
								<div class="row-list">
									<label>优先级：</label>
									<input class="td_text_w" name="priority" id="priority" type="text" class="left" value="${activityInfo.priority}"/>
								</div>
								<div class="row-list">
									<label>是否使用卡劵：</label>
								...
								<div class="row-list" id="addMallBtn">
									<label><a href="<%=basePath%>/shop/shopwinList?pageIndex=1" target="popupWin1" onclick="popupProOpen1();"><span style="margin: 0;" class="back_dblue left btn">添加门店</span></a></label>
									<div class="td_text_w" id="mall_names" style="min-height: 30px;border: 1px solid #aaa;"></div>
								</div>
								<div class="row-list">
									<label>促销标签：</label>
									<input class="td_text_w" type="text" id="promotion_label" value="${activityInfo.promotion_label}" name="promotion_label">
								</div>
							</div>
							<div class="line"></div>
							<div class="editRight">
					...
					<!--	<header class="cont_section_tab_header">商品筛选器</header>	-->
						...
						<hr>
							...
							<span><a href="<%=basePath%>/product/select?popupFlg=2&params=1" target="popupWin" onclick="popupProOpen();"><span style="display:inline-block;margin:6px 0% 0% 0%;" class="back_dblue btn"><i class="icon-btn icon-add"></i>添加商品</span></a>
						...
						<hr style="margin-top: 17px;">
							<div class="section_table">
							<table cellspacing="none">
								<span><a href="<%=basePath%>/product/select?popupFlg=2&params=2" target="popupWin" onclick="popupProOpen();"><span style="display:inline-block;margin:6px 0% 0% 0%;" class="back_dblue btn"><i class="icon-btn icon-add"></i>添加商品</span></a>
						<thead style="background:#EBEBEB;">
							<th>商品名称</th>
							<th>商品条形码</th>
							<th>销售价</th>
						...
						<tbody id="pro_item2">
						<c:forEach var="traproduct" items="${activityInfo.tradeProducts}" varStatus="status">
						 	<tr class="goods_tr">
								<td class="delbox">
								 	<input style="width:100%" type="hidden" name="product_id" readonly="readonly" value="${traproduct.product_id}"/>
								 	<input style="width:100%" type="hidden" name="p_send_type" readonly="readonly" value="${traproduct.p_send_type}"/>
								 	<input style="width:100%;" type="text" name="p_name" readonly="readonly" value="${traproduct.p_name}"/>
								</td>
								<td class="delbox">
								 	<input style="width:100%" type="text" name="p_code" readonly="readonly" value="${traproduct.p_code}"/>
								</td>
								<td class="delbox">
								 	<input style="width:100%;" type="text" name="p_price" readonly="readonly" value="${traproduct.p_price}"/>
								</td>
								<td class="delbox">
							 		<input style="width:100%;" type="text" name="p_trade_price" value="${traproduct.p_trade_price}" />
								</td>
								<td class="delbox">
							 		<input style="width:100%;" type="text" value="${traproduct.quantity}"  name="quantity" />
								</td>
							</tr>
							</c:forEach>
						</tbody>
				...
				<div class="bgbtn ">
					...
					<span ><button type="button" onclick="listDowith();" class="left back_dblue btn"><i class="icon-btn icon-save"></i>保存</button></span>
...
/**选择完重新选择高度 
 * 这个函数实在iframe子页面调用的。
 * 就是说iframe页面选择完以后，会回调这个函数，重定义高度。
 */
function funcHeight(){
