					...
					var activity_ids="";
						...
						if(activity_ids){
							...
							<%-- $.ajax({			
									...
									type:"POST",
									data: 'activity_ids='+data1+"&activity_type="+3,
									dataType: 'text',
							...
							window.location.href="<%=path%>/promotion/deleteActInfoById?activity_ids="+data1+"&activity_type=3";
						}
			}
			
			function addNew(){
				window.location.href="<%=path%>/jsp/views/activity_promotion_manage/second_half_add.jsp";
			}
	...
	<div id="wrap">
				...
				<section class="cont_section_section">					
					<div class="goodsManage">
						...
						<div class="height compa">
							<!-- <span ><input type="button" value="新增" class="left back_dblue btn" onclick="addNew()"/><input type="button" value="删除" class="left back_dblue btn" onclick="delAll()"/></span> -->
						...
						 	<tr class="goods_tr">
								<td><input style="margin-left:30%" type="checkbox" name="activity_id" id="activity_id" value="${activity.activity_id}" onclick="chooseSingle()"></td>
								...
								<td><a href="<%=path%>/promotion/selectActInfoById?activity_id=${activity.activity_id}&activity_type=${activity.activity_type}"><i class="icon-op icon-op-edit"></i>修改</a>
								&nbsp;&nbsp;
								<c:choose>  
  										<c:when test="${activity.status==0}">
