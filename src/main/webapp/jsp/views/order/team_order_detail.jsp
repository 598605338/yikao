...
<c:out value="${message }" />
				...
				<header class="cont_section_header_bread">
					...
					<a href="javascript:void(0);" class="leftNavClick" style="color: #323232;">团购订单</a><span>&gt;</span>
					<span style="color:#3bb3e0;">团购订单详情</span>
				</header>
			...
			<section class="cont_section_section_border">
					<header class="cont_section_tab_header">订单信息</header>	
						<table class="order_table">
							<tr>
								<td><label>订单号：</label><span>${orderInfo.order_id}</span></td>
								<td><label>订单状态：</label><span>${orderInfo.status}</span></td>
								<td><label>订单金额：</label><span>${orderInfo.price}</span></td>
							...
							<tr>
								...
								<td><label>运&nbsp;&nbsp;&nbsp;&nbsp;费：</label><span>${orderInfo.send_fee}</span></td>
								<td><label>订单总金额：</label><span>${orderInfo.price}</span></td>
							</tr>
							<tr>
								<td><label>支付方式：</label><span>${orderInfo.pay_type}</span></td>
					...
					<header class="cont_section_tab_header" style="margin-top:20px">客户信息</header>	
						<table class="order_table">
							<tr>
								<td style="width: 25%;"><label>用户名：</label><span>${orderInfo.cardNo}</span></td>
								<td style="width: 25%;"><label>收货人：</label><span>${orderInfo.custname}</span></td>
								<td style="width: 25%;"><label>收货电话：</label><span>${orderInfo.phone}</span></td>
								<td style="width: 25%;"><label>配送时间：</label><span>${orderInfo.send_date} ${orderInfo.send_hour}</span></td>
							</tr>
							<tr>
								<td colspan="4"><label style="float: left;text-align: right;width: 12.5%;">收货信息：</label><span style="float: left;">${orderInfo.address}</span></td>
							</tr>
						</table>
						...
						<tbody>
								<tr class="goods_tr">
									...
									<td>${product.marketPrice}-${product.ptPrice}</td>
									<td>${orderInfo.add_score}</td>
								</tr>
						</tbody>
						</table>
						</div>
