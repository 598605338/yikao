	 ...
	 function cleanForm(){
		...
		//全选功能
			...
			function addNew(ad_id){
				window.location.href="<%=path%>/jsp/views/advitise_manage/banner_add.jsp?ad_id="+ad_id;
			}
...
<div id="wrap">
<c:out value="${message }" />
				<!-- <header class="cont_section_header">Banner详情</header> -->
				<header class="cont_section_header_bread">
					...
					<span style="color:#3bb3e0;">Banner详情</span>
				</header>
				<section class="cont_section_section">					
					<div class="goodsManage">
						...
						 	<tr class="goods_tr">
								<td>${banner.order_px}</td>
								<td style="text-transform:capitalize">${banner.banner_name}</td>
								<td><img style="width:45px;" src="${banner.ban_picture}"/></td>
								<td>${banner.ban_link}</td>
								<td><a href="<%=path%>/advManage/selectBannerInfoById?id=${banner.id}">修改</a>
								&nbsp;&nbsp;<a href="<%=path%>/advManage/deleteBannerInfoById?id=${banner.id}&ad_id=${banner.ad_id}&ad_type=${banner.ad_type}">删除</a>
								</td>
							</tr>
						</c:forEach>
						</tbody>
						...
						<div class="bgbtn">
							<span class="left back_dblue btn" onclick="addNew(${ad_id})"><i class="icon-btn icon-add"></i>新增</span>
